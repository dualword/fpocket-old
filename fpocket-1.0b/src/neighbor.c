...
/**-----------------------------------------------------------------------------
   ## FUNCTION: 
	s_atm** get_lig_neigh(s_pdb *pdb, s_vsort *lsort, int *nneigh, float dist_crit) 
   -----------------------------------------------------------------------------
   ## SPECIFICATION: 
   ...
   ## PARAMETRES:
	...
	@ int *nneigh          : A pointer to the number of neighbour found, will be 
...
{	
	s_vsort *lsort =  get_sorted_list(pdb, NULL) ;	// No vertices, we only search atoms...
	...
	// Search neighbors
		while(!stopm || !stopp) {
		// Do the neighbor search while wa are in tab and we dont reach a distance up to our criteria
			ip = ntest_x + sort_x ;
			if(ip >= dim) stopp = 1 ;
			...
			if(stopp == 0) {
				...
				// OK we have an atom which is near our vertice on X, so calculate real distance
					if(dist(curap->x, curap->y, curap->z, vvalx, vvaly, vvalz) < dist_crit) {
					// Distance OK, see if the molecule is not one part of the input, and if we have not already seen it.
...
{	
	s_vsort *lsort =  get_sorted_list(pdb, lvert) ;	// No vertices, we only search atoms...
	s_vect_elem *xsort = lsort->xsort ;
...
/**-----------------------------------------------------------------------------
   ...
   ## PARAMETRES:
	@ s_atm **atoms        : The molecule.
	@ s_pdb *pdb           : The pdb structure containing all atoms of the system
	@ s_lst_vvertice *lver : A list of voronoi vertices (set it to NULL if there 
							 is no such elements
	@ float vdist_crit     : The distance criteria.
	@ int *nneigh          : A pointer to the number of neighbour found, will be 
							 modified in the function...
   -----------------------------------------------------------------------------
   ## RETURN:
							   ...
							   int interface_search, int *nneigh) 
{
	...
	int ip, im,				// Current indexes in the tab (start at the current atom position, 
							// and check the tab in each directions
		stopm, stopp,		// Say weteher the algorithm has to continue the search in each direction
		nb_neigh = 0,
	...
	// Reinitialize variables
		...
		sort_x = cur->sort_x ;		// Get the index of the current lidang atom in the sorted list.
		ntest_x = 1 ;
	...
	// Search neighbors
		while(!stopm || !stopp) {
		// Do the neighbor search while we are in tab and we dont reach a distance up to our criteria
			ip = ntest_x + sort_x ;
			if(ip >= dim) stopp = 1 ;	// If we are out of the tab, stop the search in this direction
	
			if(stopp == 0 && xsort[ip].type == M_VERTICE_TYPE) {
				curvp = (s_vvertice*) xsort[ip].data ;

				if(curvp->x - lx > vdist_crit) stopp = 1 ; // Stop when the distance reaches the criteria
				else {
				// OK we have a vertice which is near our atom on the X axe, so calculate real distance
					...
					// If the distance from the atom to the vertice is small enough
						for(j = 0 ; j < 4 ; j++) {
							...
							if(! is_in_lst_atm(neigh, nb_neigh, curatm->id)) {
								if(interface_search && dist(curatm->x, curatm->y, curatm->z, lx, ly, lz) < M_INTERFACE_SEARCH_DIST) {
			...
			im = sort_x - ntest_x ;
			if(im < 0) stopm = 1 ;		// If we are out of the tab, stop the search in this direction
